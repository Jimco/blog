  ...
  grunt.initConfig({
    ...
    transport: {
      ...
      js: {
        ...
        files: [{
          expand: true,
          cwd: 'public/js/src',              // 相对路径
          src: '*.js',                       // 需要提取依赖的文件
          dest: 'public/js/.tmp1'            // 提取后存放的临时文件
        }]
      }
    },
    // cmd 模块合并
    concat: {
      ...
      js: {
        files: [{
          expand: true,
          cwd: 'public/js/.tmp1',         // 需要合并的文件夹路径
          src: '*.js',                    // 需要合并的文件
          filter: function(filepath) {
            return !/-debug\.js$/.test(filepath); // 筛选非 -dubug.js 结尾的文件
          },
          dest: 'public/js/.tmp2',        // 合并后的存放路径
          ext: '.js'                      // 合并后的扩展名
        }]
      },
      common: {
